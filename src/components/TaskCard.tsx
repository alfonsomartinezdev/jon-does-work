      ...
      prevTasks.map((task) => {
        if (task.id === taskId) {
          if (task.isTimerActive) {
            // Stop timer - ensure timerStartTime is not null
            ...
            // Fallback if timerStartTime is somehow null
            return {
              ...task,
              isTimerActive: false,
              timerStartTime: null,
            ...
            // Start timer - first stop all other timers
            const thing = {
              ...task,
              isTimerActive: true,
        ...
        {(task.isTimerActive && task.timerStartTime) && (
          <div className="text-center">
            <div className="text-2xl font-mono font-bold text-gray-900 mb-1">
              {formatTime(Math.round((Date.now() - task.timerStartTime) / (1000)))}
            </div>
          ...
          {!(task.status == TASK_STATUS.COMPLETED) ? (
            <>
              ...
              >
                ...
                {task.isTimerActive ? "Pause" : "Start"}
              </button>
              <button
